...
為什麼不直接宣稱「毫秒級異常偵測」：
    真實 LAM 9600 資料由 Time 欄實測約 1 Hz 取樣，秒以下的物理特性沒有被記錄，
...
def load_anchor():
    ...
    source_dt = stats.get("sampling", {}).get("median_interval", 1.0)
    return {
        "start": float(prof[0]),                       # 事件起點位準
        "steady": float(prof[len(prof) // 3: -5].mean()),  # 穩態位準
        "sigma": s["within_wafer_std"],                # 殘差強度（約 1 Hz 下量到的）
        # OU 相關時間：ρ(Δt)=exp(−Δt/θ)，以實測中位取樣間隔反推。
        "theta": (-source_dt / np.log(rho1)) if 0.0 < rho1 < 1.0 else None,
...
def ou_noise(rng, n, dt, sigma, theta):
    ...
    if theta is None:                       # 原始取樣率下已無相關 → 視為白噪聲
...
def fig_detection_curves(results, source_rate_hz):
    set_plot_style()
    ...
    ax.axvline(source_rate_hz, color=COLORS["grid"], linewidth=1.2)
    ax.text(source_rate_hz * 1.02, 1.05, "← 真實資料集中位取樣率", fontsize=8,
            color=COLORS["muted"])
...
def main():
    ...
    fig_detection_curves(results, 1.0 / anchor["source_median_dt_sec"])
    ...
    for rs in ANOM_RAMP_SECS:
        ...
        if ok:
            ...
            print(f"到位 {rs}s 的異常：本掃描範圍（≤{max(RATES_HZ):g} Hz）內"
                  f"無法穩定偵測 → 需要更高取樣率")
    print("情境外插（非真實資料驗證）：0.01 秒級事件依同樣法則可能需數百 Hz；"
          "這是評估邊緣端就地取樣與推論的工程動機，不是硬體實測結論。")
